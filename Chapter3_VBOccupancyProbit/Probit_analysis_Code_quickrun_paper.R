...
#Lists that contains all of the outputs
gm1_list<-NULL #the GM VB results
vb_list<-NULL #the G VB results
vb_g_outs_list<-NULL
thin.post_list<-NULL
occupied_list<-NULL
...
#                  .inorder=T)%dopar%
{
     ...
     #{
          ...
          if (  sum(apply(Y.mat, 1, function(x) max(x, na.rm = T)))/ nrow(Y.mat) > 0.45)
          {
               ...
               gm1_list[[icounter]]<-gm1
               vb_list[[icounter]]<-vb_a1
               vb_g_outs_list[[icounter]]<-vb_g_outs
               thin.post_list[[icounter]]<-thin.post
               #occupied_list[[icounter]]<-occupied
...
save.image("SASJ2017_Probit_paper_results.RData")
#-------------------------------------------------------------------------------

#-------------------------------------------------------------------------------
#Now produce the plots for the paper
#-------------------------------------------------------------------------------

#occupancy plots

pdf("SASJ_occ_pics.pdf")
     par(mfrow=c(3,2))
     par(mar=c(3,0,0,0.5), oma=c(0,4,3,1))
     for (icounter in c(2,3,4)){
          #extract the MCMC samples
          beta0<-thin.post_list[[icounter]][,1]
          beta1<-thin.post_list[[icounter]][,2]
          
          #plots for beta0
          p1=density(beta0)
          xx=seq(from=min(p1$x), to=max(p1$x), length.out=1000)
          d1=dnorm(xx, mean=vb_list[[icounter]]$beta[1], sd=sqrt(vb_list[[icounter]]$Sigma_beta[1,1]))
          hip1=max(p1$y)
          hip2=max(d1)
          h<-hist(beta0, plot=F, breaks=50, prob=T)
          #hist(beta0, plot=T, breaks=50, col="white", prob=T, xlab=expression(beta[0]), main="", ylab="Density",ylim=c(0,max(hip1,hip2)))
          plot(h$breaks,c(h$density,0),type="s",col="black", ylim=c(0,max(hip1,hip2)), xlab=expression(beta[0]), main="", ylab="Density", yaxt="n")
          lines(xx, d1, col="red")
          legend("topright", c("G","GM"), lwd=c(1,2), col=c("red","blue"),bty="n")
          p1gm=marg.gm2(x=seq(from=min(beta0), to=max(beta0), length.out=10002), gm=gm1_list[[icounter]]$iter_gm.b, index=1)
          lines(p1gm$x, p1gm$y, col="blue", lwd=2)
          
          #plots for beta1
          p1=density(beta1)
          xx=seq(from=min(p1$x), to=max(p1$x), length.out=1000)
          d1=dnorm(xx, mean=vb_list[[icounter]]$beta[2], sd=sqrt(vb_list[[icounter]]$Sigma_beta[2,2]))
          hip1=max(p1$y)
          hip2=max(d1)
          #h<-hist(beta1, plot=F, breaks=50, col="yellow", prob=T, xlab=expression(beta[1]), main="", ylab="Density")
          #hist(beta1, plot=T, breaks=50, col="white", prob=T, xlab=expression(beta[1]), main="", ylab="Density", ylim=c(0,max(hip1,hip2)))
          h<-hist(beta1, plot=F, breaks=50)
          plot(h$breaks,c(h$density,0),type="s",col="black", ylim=c(0,max(hip1,hip2)), xlab="AETPET", main="", ylab="Density", yaxt="n")
          lines(xx, d1, col="red")
          
          if (icounter==2){
               legend("topleft", c("G", "GM"), lwd=c(1,2), col=c("red","blue"),bty="n")}else
          {
               legend("topright", c("G", "GM"), lwd=c(1,2), col=c("red","blue"),bty="n")     
          }
          p1gm=marg.gm2(x=seq(from=min(beta1), to=max(beta1), length.out=10002), gm=gm1_list[[icounter]]$iter_gm.b, index=2)
          lines(p1gm$x, p1gm$y, col="blue", lwd=2)
          
     }
     
     mtext(expression(beta[0]), outer=TRUE, line=1, side=3, col="black", adj=0.22, font=2, cex=1)
     mtext("AETPET", outer=TRUE, line=1, side=3, col="black", adj=0.75, font=2, cex=1)
     
     #when using 4 species
     #mtext("(4)", outer=TRUE, line=1, side=2, col="black", adj=0.15, font=2, cex=1)
     #mtext("(3)", outer=TRUE, line=1, side=2, col="black", adj=0.4, font=2, cex=1)
     #mtext("(2)", outer=TRUE, line=1, side=2, col="black", adj=0.67, font=2, cex=1)
     #mtext("(1)", outer=TRUE, line=1, side=2, col="black", adj=0.93, font=2, cex=1)
     
     mtext("(3)", outer=TRUE, line=1, side=2, col="black", adj=0.18, font=2, cex=1)
     mtext("(2)", outer=TRUE, line=1, side=2, col="black", adj=0.54, font=2, cex=1)
     mtext("(1)", outer=TRUE, line=1, side=2, col="black", adj=0.88, font=2, cex=1)
dev.off()
#-------------------------------------------------------------------------------

pdf("SASJ_det_pics.pdf")
     #detection plots
     par(mfrow=c(3,2))
     par(mar=c(3,0,0,0.5), oma=c(0,4,3,1))
     for (icounter in 2:4){
          #extract the MCMC samples
          alpha0<- thin.post_list[[icounter]][,3]    
          alpha1<-thin.post_list[[icounter]][,4]
          
          p1=density(alpha0)
          xx=seq(from=(min(p1$x)-.25), to=(max(p1$x)+.25), length.out=1000)
          d1=dnorm(xx, mean=vb_list[[icounter]]$alpha[1], sd=sqrt(vb_list[[icounter]]$Sigma_alpha[1,1]))
          hip1=max(p1$y)
          hip2=max(d1)
          h<-hist(alpha0, plot=F, breaks=50)
          plot(h$breaks,c(h$density,0),type="s",col="black", ylim=c(0,max(hip1,hip2)), xlab=expression(alpha[0]), main="", ylab="Density", yaxt="n")
          lines(xx, d1, col="red")
          legend("topleft", c("G","GM"), lwd=c(1,2), col=c("red","blue"),bty="n")
          p1gm=marg.gm2(x=seq(from=min(alpha0), to=max(alpha0), length.out=10002), gm=gm1_list[[icounter]]$iter_gm.a, index=1)
          lines(p1gm$x, p1gm$y, col="blue", lwd=2)
          
          #plots for alpha1
          p1=density(alpha1)
          xx=seq(from=min(p1$x), to=max(p1$x), length.out=1000)
          d1=dnorm(xx, mean=vb_list[[icounter]]$alpha[2], sd=sqrt(vb_list[[icounter]]$Sigma_alpha[2,2]))
          hip1=max(p1$y)
          hip2=max(d1)
          h<-hist(alpha1, plot=F, breaks=50)
          plot(h$breaks,c(h$density,0),type="s",col="black", ylim=c(0,max(hip1,hip2)), xlab="nspp", main="", ylab="Density", yaxt="n")
          lines(xx, d1, col="red")
          legend("topleft", c("G","GM"), lwd=c(1,2), col=c("red","blue"),bty="n")
          p1gm=marg.gm2(x=seq(from=min(alpha1), to=max(alpha1), length.out=10002), gm=gm1_list[[icounter]]$iter_gm.a, index=2)
          lines(p1gm$x, p1gm$y, col="blue", lwd=2)
     }
     
     mtext(expression(alpha[0]), outer=TRUE, line=1, side=3, col="black", adj=0.22, font=2, cex=1)
     mtext("nspp", outer=TRUE, line=1, side=3, col="black", adj=0.75, font=2, cex=1)
...
#function to calculate mean of Gaussian linear mixture
stats_GM<-function(gm){
     weights_n_o<-length(gm$iter_gm.b$weights)
     weights_n_d<-length(gm$iter_gm.a$weights)
     
     mean_o<- apply( gm$iter_gm.b$weights* gm$iter_gm.b$means , 2, sum)
     mean_d<- apply( gm$iter_gm.a$weights* gm$iter_gm.a$means , 2, sum)
     
     cov_o<-matrix(0, nrow=2, ncol=2)
     
     for (i in 1:weights_n_o){
          cov_o <- cov_o + gm$iter_gm.b$weights[i]*( gm$iter_gm.b$sigmas[[i]] + crossprod(t(gm$iter_gm.b$mean[i,])) )
     }
     cov_o <- cov_o - crossprod(t(mean_o))

     cov_d<-matrix(0, nrow=2, ncol=2)
     
     for (i in 1:weights_n_d){
          cov_d <- cov_d + gm$iter_gm.a$weights[i]*( gm$iter_gm.a$sigmas[[i]] + crossprod(t(gm$iter_gm.a$mean[i,])) )
     }
     cov_d <- cov_d - crossprod(t(mean_d))
     
     return( cbind( c( mean_o, mean_d), 
            c(sqrt( diag(cov_o )), c(sqrt( diag(cov_d ) ))) ) )
     
}


#the Posterior stats for the three methods
Outs<-matrix(0, ncol=7, nrow=1)

for (icounter in 2:4){
     #G stats
     vb_la_outs<-vb_g_outs_list[[icounter]] #beta0, beta1, alpha0, alpha1
     ...
     #mcmc stats
     mcmc_stats<-cbind(
          c(mean(beta0), mean(beta1), mean(alpha0), mean(alpha1)), 
          c(sd(beta0), sd(beta1), sd(alpha0), sd(alpha1)),
          c(median(beta0), median(beta1), median(alpha0), median(alpha1)) )
     
     gm_stats<- stats_GM( gm1_list[[icounter]] )
     
     outs<- cbind( vb_la_outs[,1:2], gm_stats, mcmc_stats[,1:3] )
     colnames(outs)<- c(rep(c("Mean", "Std"),3), "Median")
     
     Outs<-rbind(Outs, outs)
}

Outs<-Outs[-1,]
xtable(Outs, digits=rep(3,8))

#remove some of the recycled entries in the table!
xtable(rbind(
     gm1_list[[2]]$iter_gm.a$weights,
     gm1_list[[3]]$iter_gm.a$weights,
     gm1_list[[4]]$iter_gm.a$weights,
     gm1_list[[2]]$iter_gm.b$weights,
gm1_list[[3]]$iter_gm.b$weights,
gm1_list[[4]]$iter_gm.b$weights), digit=rep(3,7))
#-------------------------------------------------------------------------------

#What about PAO
#Obtain the data for species == 200
icounter<-0
for (ispecies in 200)  
{
     icounter<-icounter+1
...
#Approximations to PAO
for (icounter in 4){
     occupiedmcmc<-thin.post_list[[icounter]][,5]*dim(Xmat)[1] #mcmc
     cat("\n Completed MCMC of PAO")
     #--------------------------------------------------------------------------
     
     #G 
     Occpla<-vb_list[[icounter]]$occup_p
     Nsim=50000
     ...
     cat("\n Completed G approximation of PAO")
     #--------------------------------------------------------------------------
     
     #GM
     OccpGM<-gm1_list[[icounter]]$occup_p
     occ_n<-matrix(1,ncol=Nsim, nrow=length(OccpGM))
     zmaybe<-which(apply(Y.mat,1,function(x)max(x,na.rm=T))==0) #only sample at these places
     
     for (i in zmaybe){occ_n[i,]<-rbinom(Nsim, p=OccpGM[i], size=1)}
     
     occupiedGM<-apply(occ_n, 2, sum)
     cat("\n Completed GM approximation of PAO")
     #--------------------------------------------------------------------------
     
     
     #sample from the occupancy and detection regression effects
     reg_s<-50000
     sample_a=sample(x=c(1:length(gm1_list[[icounter]]$iter_gm.a$weights)), size=reg_s, prob=gm1_list[[icounter]]$iter_gm.a$weights , replace=TRUE)
     sample_b=sample(x=c(1:length(gm1_list[[4]]$iter_gm.b$weights)), size=reg_s, prob=gm1_list[[4]]$iter_gm.b$weights , replace=TRUE)

     #vectors that contains the simulated regression effects
     reg_o_samples<-matrix(0, nrow=reg_s, ncol=2)
     reg_d_samples<-reg_o_samples
     PAO_s <- reg_o_samples[,1]
     
     det_p <- nspp #same size as nspp
     occ_p <- matrix(1, ncol=1, nrow=NROW(nspp))
     s_isite <- occ_p
     z_isite <- occ_p
     
     #really inefficient! but works
     for (b_samples in 1:reg_s){
          reg_o_samples[b_samples,]<- mvrnorm(n = 1, mu = gm1_list[[icounter]]$iter_gm.b$means[sample_b[b_samples],], Sigma=gm1_list[[icounter]]$iter_gm.b$sigmas[[sample_b[b_samples]]], tol = 1e-6, empirical = FALSE, EISPACK = FALSE) 
     
          reg_d_samples[b_samples,]<- mvrnorm(n = 1, mu = gm1_list[[icounter]]$iter_gm.a$means[sample_a[b_samples],], Sigma=gm1_list[[icounter]]$iter_gm.a$sigmas[[sample_a[b_samples]]], tol = 1e-6, empirical = FALSE, EISPACK = FALSE) 
          
          #the occupancy probs
          for (isite in 1:NROW(occ_p)){
               if (design_mats$pres_abs[isite]==0){
                    temp_o <- exp( reg_o_samples[b_samples,1] + Xmat$AET_div_PET_s[isite] * reg_o_samples[b_samples,2])
                    occ_p[isite]<- temp_o/(1+temp_o)
               }
               
               #the detection probs
               temp_d_ij<-1
                         
               for (ivisit in 1:design_mats$nvisits[isite]){
                    temp_d <- exp( reg_d_samples[b_samples,1] + nspp[isite, ivisit] * reg_d_samples[b_samples,2])                         
                    det_p[isite, ivisit] <- temp_d/(1+temp_d)
                    temp_d_ij<- temp_d_ij*(1-det_p[isite, ivisit])
               }
                         
               s_isite[isite] <- (temp_d_ij*occ_p[isite])/(1-occ_p[isite] + temp_d_ij*occ_p[isite]) #site occupancy prob!
               z_isite[isite] <- rbinom(n=1, size = 1, prob = s_isite[isite] )
               
               #print(z_isite)
          }#end of sampling for the sites
          
          PAO_s[b_samples] <- sum(z_isite)
          if ( (floor(b_samples/500) - b_samples/500) == 0){cat("\n b_samples=", b_samples, "PAO=",PAO_s[b_samples])}
     }#end of sampling from the Gaussian linear model
}

table(occupiedmcmc)/length(occupiedmcmc)
table(PAO_s)/length(PAO_s)
table(occupiedla)/length( occupiedla )
table(occupiedGM)/length( occupiedGM )

#remove 
rbind(
table(occupiedmcmc)/length(occupiedmcmc),
c(table(PAO_s)/length(PAO_s),0),
c(table(occupiedla)/length( occupiedla ), rep(0,3)),
c(table(occupiedGM)/length( occupiedGM ), rep(0,2))
)

mcmc.prob<-table(occupiedmcmc)/length(occupiedmcmc)
hybrid<-c(table(PAO_s)/length(PAO_s),0)
PAO_G<-c(table(occupiedla)/length( occupiedla ), rep(0,3))
PAO_GM<-c(table(occupiedGM)/length( occupiedGM ), rep(0,2))

#barplot of mcmc and VB methods
pdf("PAO_SRBishop.pdf")
     par(mfrow=c(1,1), mar=c(4,3,1,1))
     data<-rbind(mcmc.prob,hybrid, PAO_G, PAO_GM)
     colnames(data) <- paste(102:110)
     barplot(data, beside=TRUE, space=c(0,2), las=1, col=c("black", "grey","white", "red"), xlab="Number of occupied sites", axes="FALSE", ylim=c(0,.4), density=c(100,20, 0, 30), angle=c(0,10,0,60))
     axis(2, at=seq(0, 0.4, by=.1), las=1, cex.axis=0.8)
     legend("topright", c("MCMC","VB - Hybrid","VB - G","VB - GM"), fill=c("black", "grey","white", "red"), bty="n", density=c(100,20,0,30), angle=c(0,10,0,60))
dev.off()
