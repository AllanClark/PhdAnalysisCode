...
DA<-function(X, W, Y, J, ndraws=25000, alpha.starts, beta.starts,nburn=0.5*ndraws,thin=1)
{
     ...
     for (draw in 1:ndraws) {
          if (draw == 0.5*ndraws)  cat('halfway done..... drawing sample #', draw, CR)
     ...
     cat('Gibbs sampling is completed!', CR, CR)
...
use.species<-13
bayes.species<-NULL
sink1<-"Analysis2_3_to_5_2012_Bayes2.txt"
what.year=112
...
#                  .inorder=T)%dopar%
{
     ...
     #{
          ...
          Y.mat<-Y 
          if (  sum(apply(Y.mat, 1, function(x) max(x, na.rm = T)))/ nrow(Y.mat) > 0.35)
          {  
               #------------------------------------------------------------------
               #the VB fit
               #------------------------------------------------------------------
     ...
     #check           
               ...
               RV<-cbind(v1v2, mcmctab)
               #print(RV)
               print(xtable(RV, digits=3, align=c("|l|","c|","c|","c","c","c|")))
               
               print("The outputs associated with the GM fit for alpha")
               gm1$iter_gm.a$weights
               print("Means for components are stored row-wise!")
               gm1$iter_gm.a$means
               gm1$iter_gm.a$sigmas
               
               print("The outputs associated with the GM fit for beta")
               gm1$iter_gm.b$weights
               print("Means for components are stored row-wise!")
               gm1$iter_gm.b$means
               gm1$iter_gm.b$sigmas
